...
class KeywordHarvester:
            ...
            with open(csv_path, "r", encoding="utf-8") as f:
                ...
                for r in reader:
                    if r.get("status") == "ready":
                        ...
                        if not keyword:
                            continue
                        category = r.get("category", "정부지원금 & 복지")
    ...
    def harvest_ideas(self, target_category: str = None) -> List[Dict[str, Any]]:
        ...
        category_name = selected_cat["name"] if selected_cat else "정부지원금 & 복지"
        seed_keywords = selected_cat.get("keywords", []) if selected_cat else []
